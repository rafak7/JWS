...
interface ReportConfig {
  ...
  // Campos específicos para Mark1
  includeMark1Company?: boolean;
  includeMark1Address?: boolean;
  includeMark1Date?: boolean;
  includeMark1Location?: boolean;
  includeMark1StartTime?: boolean;
  includeMark1EndTime?: boolean;
}
...
interface Mark1ProcessReportData {
  workName: string;
  workDate: string;
  company: string;
  address: string;
...
export default function AdminDashboard() {
  ...
  const [mark1Data, setMark1Data] = useState<Mark1Data>({
    ...
    config: {
      ...
      includeFinalConsiderations: true,
      includeProcessImages: true,
      // Campos específicos para Mark1 - todos ativos por padrão
      includeMark1Company: true,
      includeMark1Address: true,
      includeMark1Date: true,
      includeMark1Location: true,
      includeMark1StartTime: true,
      includeMark1EndTime: true
    },
    finalConsiderations: ''
  });
  const [activeServiceId, setActiveServiceId] = useState('1');
  const [activePremiereServiceId, setActivePremiereServiceId] = useState('1');
  const [activeMtaServiceId, setActiveMtaServiceId] = useState('1');
  ...
  const [mtaLocation, setMtaLocation] = useState('Local da Obra');
  const [mtaReportName, setMtaReportName] = useState('Relatório de Obra MTA');
  const [mtaReportDescription, setMtaReportDescription] = useState('');
  ...
  const [mark1ReportTitle, setMark1ReportTitle] = useState('Relatório Diário de Obras');
  const [processReportData, setProcessReportData] = useState<ProcessReportData>({
    workName: '',
    workDate: new Date().toISOString().split('T')[0],
    processImages: []
  });
  const [mark1ProcessReportData, setMark1ProcessReportData] = useState<Mark1ProcessReportData>({
    workName: '',
    workDate: new Date().toISOString().split('T')[0],
    company: '',
    address: '',
  ...
  // Funções para o relatório de Processo Mark1
  const handleMark1ProcessReportImageUpload = (e: React.ChangeEvent<HTMLInputElement>, phase: 'antes' | 'durante' | 'depois') => {
    const files = Array.from(e.target.files || []);
    const currentDate = new Date().toISOString().split('T')[0];
    const newImages = files.map(file => ({ file, comment: '', phase, captureDate: currentDate }));
    setMark1ProcessReportData(prev => ({
      ...prev,
      processImages: [...prev.processImages, ...newImages]
    }));
  };

  const removeMark1ProcessReportImage = (index: number) => {
    setMark1ProcessReportData(prev => ({
      ...prev,
      processImages: prev.processImages.filter((_, i) => i !== index)
    }));
  ...
  const updateMark1ProcessReportImageComment = (imageIndex: number, comment: string) => {
    setMark1ProcessReportData(prev => ({
      ...prev,
      processImages: prev.processImages.map((img, index) =>
        index === imageIndex
  ...
  const getMark1ProcessReportImagesByPhase = (phase: 'antes' | 'durante' | 'depois') => {
    return mark1ProcessReportData.processImages.filter(img => img.phase === phase);
  };

  const generateMark1ProcessReportPDF = async () => {
    if (mark1ProcessReportData.processImages.length === 0) {
      setMessage('Adicione pelo menos uma imagem para gerar o relatório.');
      return;
    }

    if (!mark1ProcessReportData.workName.trim()) {
    ...
    try {
      ...
      formData.append('workName', mark1ProcessReportData.workName);
      formData.append('workDate', mark1ProcessReportData.workDate);
      formData.append('company', mark1ProcessReportData.company);
      formData.append('address', mark1ProcessReportData.address);
      formData.append('location', mark1ProcessReportData.location);
      formData.append('description', mark1ProcessReportData.description);

      // Adicionar imagens do processo
      let processImageIndex = 0;
      mark1ProcessReportData.processImages.forEach((processImageData) => {
      ...
      console.log('FormData criado para relatório de processo Mark1, enviando requisição...');
      ...
      const response = await fetch('/api/admin/generate-mark1-process-report', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      a.href = url;
      a.download = `processo-obra-mark1-${mark1ProcessReportData.workName.replace(/[^a-zA-Z0-9]/g, '-')}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);

      setMessage('Relatório de Processo Mark1 gerado com sucesso!');
    } catch (error: any) {
      console.error('Erro ao gerar PDF do processo da obra Mark1:', error);
  ...
  const handlePremiereTermsOfServiceImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setPremiereData(prev => ({ ...prev, termsOfServiceImage: file }));
    }
  };

  const removePremiereTermsOfServiceImage = () => {
    setPremiereData(prev => ({ ...prev, termsOfServiceImage: null }));
  };
  ...
  // Funções para gerenciar serviços do MTA
  const addMtaService = () => {
    const newId = Date.now().toString();
    setMtaData(prev => ({
      ...prev,
    ...
    setActiveMtaServiceId(newId);
  };

  const removeMtaService = (serviceId: string) => {
    if (mtaData.services.length <= 1) return;

    const newServices = mtaData.services.filter(service => service.id !== serviceId);
    setMtaData(prev => ({
      ...prev,
      services: newServices
    }));

    if (activeMtaServiceId === serviceId) {
      setActiveMtaServiceId(newServices[0].id);
    }
  };

  const updateMtaServiceName = (serviceId: string, name: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const updateMtaServiceStartDate = (serviceId: string, startDate: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const updateMtaServiceEndDate = (serviceId: string, endDate: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const updateMtaServiceObservations = (serviceId: string, observations: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const handleMtaImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;
  ...
  const removeMtaImage = (serviceId: string, index: number) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const removeAllMtaImagesFromService = (serviceId: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const updateMtaImageComment = (serviceId: string, imageIndex: number, comment: string) => {
    setMtaData(prev => ({
      ...prev,
      services: prev.services.map(service =>
        service.id === serviceId
  ...
  const updateMtaConfig = (key: keyof ReportConfig, value: boolean) => {
    setMtaData(prev => ({
      ...prev,
      config: { ...prev.config, [key]: value }
    }));
  ...
  const updateMtaFinalConsiderations = (text: string) => {
    setMtaData(prev => ({
      ...prev,
      finalConsiderations: text
    }));
  ...
  const handleMtaResultImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setMtaData(prev => ({ ...prev, resultImage: file }));
    }
  };

  const removeMtaResultImage = () => {
    setMtaData(prev => ({ ...prev, resultImage: null }));
  };

  const handleMtaTermsOfServiceImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setMtaData(prev => ({ ...prev, termsOfServiceImage: file }));
    }
  };

  const removeMtaTermsOfServiceImage = () => {
    setMtaData(prev => ({ ...prev, termsOfServiceImage: null }));
  };

  const getActiveMtaService = () => {
    return mtaData.services.find(service => service.id === activeMtaServiceId);
  };

  const getTotalMtaImages = () => {
    return mtaData.services.reduce((total, service) => total + service.images.length, 0);
  };
  ...
  const updateMark1Config = (key: keyof ReportConfig, value: boolean) => {
    console.log(`Atualizando configuração Mark1: ${key} = ${value}`);
    setMark1Data(prev => ({
      ...prev,
      config: { ...prev.config, [key]: value }
    }));
  ...
  const generateMark1PDF = async (preview: boolean = false) => {
    setLoading(true);
    setMessage('');
    ...
    try {
      ...
      formData.append('reportTitle', mark1ReportTitle);

      console.log('FormData base criado');
      console.log('Configurações Mark1:', mark1Data.config);
      ...
      if (response.ok) {
        ...
        if (preview) {
          ...
          setMessage('Pré-visualização gerada com sucesso!');
        } else {
          const a = document.createElement('a');
          a.href = url;
  ...
  const generatePremierePDF = async (preview: boolean = false) => {
    setLoading(true);
    setMessage('');
    ...
    try {
      ...
      console.log('FormData base criado');
      console.log('DEBUG: Iniciando processamento de imagens por serviço...');
      console.log('DEBUG: Total de serviços:', premiereData.services.length);

      let imageIndex = 0;
      premiereData.services.forEach((service, serviceIdx) => {
        console.log(`DEBUG: Serviço ${serviceIdx + 1}:`, {
          id: service.id,
          name: service.name,
          totalImages: service.images.length
        });

        service.images.forEach((imageData, imgIdx) => {
          console.log(`  DEBUG: Imagem ${imgIdx + 1}/${service.images.length}:`, {
      ...
      if (premiereData.termsOfServiceImage) {
        formData.append('termsOfServiceImage', premiereData.termsOfServiceImage);
        console.log('Imagem do termo de serviço Premiere adicionada');
      }
  ...
  const generateMtaPDF = async (preview: boolean = false) => {
    setLoading(true);
    setMessage('');
    ...
    try {
      ...
      const formData = new FormData();
      formData.append('services', JSON.stringify(mtaData.services));
      formData.append('config', JSON.stringify(mtaData.config));
      formData.append('finalConsiderations', mtaData.finalConsiderations);
      formData.append('location', mtaLocation);
      formData.append('reportName', mtaReportName);
      formData.append('reportDescription', mtaReportDescription);

      console.log('FormData base criado (MTA)');

      let imageIndex = 0;
      mtaData.services.forEach((service) => {
        service.images.forEach((imageData) => {
          formData.append(`image_${imageIndex}`, imageData.file);
          formData.append(`image_${imageIndex}_serviceId`, service.id);
          formData.append(`image_${imageIndex}_comment`, imageData.comment);
      ...
      if (mtaData.resultImage) {
        formData.append('resultImage', mtaData.resultImage);
      }

      if (mtaData.termsOfServiceImage) {
        formData.append('termsOfServiceImage', mtaData.termsOfServiceImage);
      }
      ...
      const response = await fetch('/api/admin/generate-mta-report', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      if (response.ok) {
        ...
        if (preview) {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `diario_obra_mta_${Date.now()}.pdf`;
          document.body.appendChild(a);
          a.click();
          window.URL.revokeObjectURL(url);
          document.body.removeChild(a);
          setMessage('Diário de Obra MTA gerado com sucesso!');
        }
      } else {
        let errorMessage = 'Erro ao gerar relatório MTA';
        try {
          const error = await response.json();
          errorMessage = error.message || errorMessage;
        } catch (e) {
      ...
      console.error('Erro ao gerar relatório MTA:', error);
      if (error.name === 'AbortError') {
        setMessage('Timeout: O processamento demorou muito. Tente com menos imagens.');
      } else {
        setMessage(`Erro ao gerar relatório MTA: ${error.message || 'Erro desconhecido'}`);
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 w-full">
        <div className="px-4 py-6 sm:px-0 h-full">
          <Card className="relative min-h-0 w-full">
            ...
            <CardContent>
              <Tabs defaultValue="relatorio" className="w-full flex flex-col md:flex-row gap-6">
                <aside className="w-full md:w-64 flex-shrink-0">
                  <div className="sticky top-6">
                    <TabsList className="flex flex-col h-auto bg-gray-100/50 p-2 space-y-2 w-full rounded-lg">
                      <TabsTrigger value="relatorio" className="justify-start w-full px-4 py-3 data-[state=active]:bg-white data-[state=active]:shadow-sm">
                        <FileText className="w-4 h-4 mr-2" />
                        Relatório de Obra
                      </TabsTrigger>
                      <TabsTrigger value="processo" className="justify-start w-full px-4 py-3 data-[state=active]:bg-white data-[state=active]:shadow-sm">
                        <Camera className="w-4 h-4 mr-2" />
                        Processo da Obra
                      </TabsTrigger>
                      <TabsTrigger value="premiere" className="justify-start w-full px-4 py-3 data-[state=active]:bg-blue-100 data-[state=active]:text-blue-700 data-[state=active]:shadow-sm">
                        <FileText className="w-4 h-4 mr-2" />
                        Premiere
                      </TabsTrigger>
                      <TabsTrigger value="mta" className="justify-start w-full px-4 py-3 data-[state=active]:bg-green-100 data-[state=active]:text-green-700 data-[state=active]:shadow-sm">
                        <FileText className="w-4 h-4 mr-2" />
                        MTA
                      </TabsTrigger>
                      <TabsTrigger value="mark1" className="justify-start w-full px-4 py-3 data-[state=active]:bg-orange-100 data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">
                        <FileText className="w-4 h-4 mr-2" />
                        Mark1
                      </TabsTrigger>
                      <TabsTrigger value="processo-mark1" className="justify-start w-full px-4 py-3 data-[state=active]:bg-orange-100 data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">
                        <Camera className="w-4 h-4 mr-2" />
                        Processo Mark1
                      </TabsTrigger>
                      <TabsTrigger value="cronograma" className="justify-start w-full px-4 py-3 data-[state=active]:bg-white data-[state=active]:shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                          ...
                          {reportData.services.map((service, index) => (
                            <div key={service.id} className="flex items-center">
                              <button
                                ...
                                className={`px-4 py-2 text-sm font-medium whitespace-nowrap max-w-xs truncate ${activeServiceId === service.id
                                    ...
                                    <div key={index} className="space-y-2">
                                      <div className="relative">
                                        <img
                                          ...
                                          alt={`Imagem ${index + 1}`}
                                          className={`w-full h-24 object-cover rounded-lg ${index >= 20 ? 'opacity-50' : ''
                                            }`}
                                        />
                                        {index >= 20 && (
                  ...
                  <TabsContent value="premiere" className="space-y-6 mt-6">
                          ...
                          {premiereData.services.map((service, index) => (
                            <div key={service.id} className="flex items-center">
                              <button
                                ...
                                className={`px-4 py-2 text-sm font-medium whitespace-nowrap max-w-xs truncate ${activePremiereServiceId === service.id
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                                {getActivePremiereService()?.images.map((imageData, index) => (
                                  <div key={index} className="border rounded-lg overflow-hidden group">
                                    ...
                                    <div className="p-2">
                                      <Textarea
                                        placeholder="Comentário da imagem (agora aparece abaixo da foto no PDF)..."
                                        value={imageData.comment}
                    ...
                    {/* Termo de Serviço Premiere */}
                    <div className="space-y-4">
                      <Label className="text-lg font-semibold">Termo de Serviço (Opcional)</Label>
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-purple-400 transition-colors">
                        {premiereData.termsOfServiceImage ? (
                          <div className="space-y-4">
                            <img
                              src={URL.createObjectURL(premiereData.termsOfServiceImage)}
                              alt="Termo de Serviço"
                              className="mx-auto max-h-96 rounded-lg border-2 border-purple-200"
                            />
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={removePremiereTermsOfServiceImage}
                              className="text-red-600 hover:text-red-700"
                            >
                              <X className="w-4 h-4 mr-2" />
                              Remover Termo
                          ...
                          <>
                            ...
                            <p className="mt-2 text-sm text-gray-600">
                              Adicione uma imagem do termo de serviço para assinatura
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              A imagem será exibida em página dedicada, ocupando quase toda a tela com bordas nas cores do layout
                            </p>
                            <Input
                              type="file"
                              accept="image/*"
                              onChange={handlePremiereTermsOfServiceImageUpload}
                    ...
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-6">
                      ...
                      <div className="flex gap-2">
                        <Button
                          onClick={() => generatePremierePDF(true)}
                          disabled={loading || premiereData.services.length === 0}
                          className="flex-1 bg-purple-100 text-purple-700 hover:bg-purple-200 border border-purple-200"
                          size="lg"
                          variant="outline"
                        >
                        ...
                        <Button
                          onClick={() => generatePremierePDF(false)}
                          disabled={loading || premiereData.services.length === 0}
                  ...
                  <TabsContent value="mta" className="space-y-6 mt-6">
                    {/* Seção de Configuração do Relatório MTA */}
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <Label className="text-lg font-semibold">Configuração do Relatório MTA</Label>
                        <span className="text-sm text-gray-500">
                          {Object.values(mtaData.config).filter(Boolean).length} de {Object.keys(mtaData.config).length} elementos ativos
                        </span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-green-50 rounded-lg border-l-4 border-green-500">
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeCompanyHeader"
                            checked={mtaData.config.includeCompanyHeader}
                            onChange={(e) => updateMtaConfig('includeCompanyHeader', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeCompanyHeader" className="text-sm">Cabeçalho da Empresa</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeServicesList"
                            checked={mtaData.config.includeServicesList}
                            onChange={(e) => updateMtaConfig('includeServicesList', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeServicesList" className="text-sm">Lista de Serviços</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeServiceDates"
                            checked={mtaData.config.includeServiceDates}
                            onChange={(e) => updateMtaConfig('includeServiceDates', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeServiceDates" className="text-sm">Datas dos Serviços</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeServiceObservations"
                            checked={mtaData.config.includeServiceObservations}
                            onChange={(e) => updateMtaConfig('includeServiceObservations', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeServiceObservations" className="text-sm">Observações dos Serviços</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeImageComments"
                            checked={mtaData.config.includeImageComments}
                            onChange={(e) => updateMtaConfig('includeImageComments', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeImageComments" className="text-sm">Comentários das Imagens</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeResultImage"
                            checked={mtaData.config.includeResultImage}
                            onChange={(e) => updateMtaConfig('includeResultImage', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeResultImage" className="text-sm">Imagem de Resultado</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includePhotographicReport"
                            checked={mtaData.config.includePhotographicReport}
                            onChange={(e) => updateMtaConfig('includePhotographicReport', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includePhotographicReport" className="text-sm">Texto "Relatório Fotográfico"</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeHeaderFooter"
                            checked={mtaData.config.includeHeaderFooter}
                            onChange={(e) => updateMtaConfig('includeHeaderFooter', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeHeaderFooter" className="text-sm">Header e Footer com Logo</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeFinalConsiderations"
                            checked={mtaData.config.includeFinalConsiderations}
                            onChange={(e) => updateMtaConfig('includeFinalConsiderations', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeFinalConsiderations" className="text-sm">Considerações Finais</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="mta-includeProcessImages"
                            checked={mtaData.config.includeProcessImages}
                            onChange={(e) => updateMtaConfig('includeProcessImages', e.target.checked)}
                            className="rounded text-green-600 focus:ring-green-500"
                          />
                          <Label htmlFor="mta-includeProcessImages" className="text-sm">Processo da Obra (Antes/Durante/Depois)</Label>
                        </div>
                      </div>

                      {Object.values(mtaData.config).filter(Boolean).length < 3 && (
                        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                          <p className="text-sm text-yellow-700">
                            ⚠️ Atenção: Poucas opções estão ativas. Verifique se o relatório terá o conteúdo necessário.
                          </p>
                    ...
                    {/* Seção de Serviços MTA */}
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <Label className="text-lg font-semibold">Serviços MTA ({mtaData.services.length})</Label>
                        <Button onClick={addMtaService} variant="outline" size="sm" className="border-green-200 hover:bg-green-50 text-green-700">
                          <Plus className="w-4 h-4 mr-2" />
                          Adicionar Serviço
                        </Button>
                      </div>

                      {/* Abas dos Serviços MTA */}
                      <div className="border rounded-lg border-green-200">
                        <div className="flex border-b overflow-x-auto bg-green-50/30">
                          {mtaData.services.map((service, index) => (
                            <div key={service.id} className="flex items-center">
                              <button
                                onClick={() => setActiveMtaServiceId(service.id)}
                                className={`px-4 py-2 text-sm font-medium whitespace-nowrap max-w-xs truncate ${activeMtaServiceId === service.id
                                  ? 'bg-green-100 text-green-800 border-b-2 border-green-700'
                              ...
                              >
                                ...
                                {service.images.length > 0 && (
                                  <span className="ml-2 bg-green-200 text-green-900 text-xs px-2 py-1 rounded-full flex-shrink-0">
                                    {service.images.length}
                                  </span>
                                )}
                              </button>
                              {mtaData.services.length > 1 && (
                                <button
                                  onClick={() => removeMtaService(service.id)}
                        ...
                        {/* Conteúdo do Serviço Ativo MTA */}
                        <div className="p-4 space-y-4">
                          <div className="space-y-2">
                            <Label htmlFor="mta-service-name">Nome do Serviço</Label>
                            <Input
                              id="mta-service-name"
                              placeholder="Ex: Instalação de Drywall, Pintura, etc."
                              value={getActiveMtaService()?.name || ''}
                              onChange={(e) => updateMtaServiceName(activeMtaServiceId, e.target.value)}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label htmlFor="mta-start-date">Data de Início</Label>
                              <Input
                                id="mta-start-date"
                                type="date"
                                disabled={!mtaData.config.includeServiceDates}
                                value={getActiveMtaService()?.startDate || ''}
                                onChange={(e) => updateMtaServiceStartDate(activeMtaServiceId, e.target.value)}
                                className="focus:ring-green-500 focus:border-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                              />
                            </div>
                            <div className="space-y-2">
                              <Label htmlFor="mta-end-date">Data de Término</Label>
                              <Input
                                id="mta-end-date"
                                type="date"
                                disabled={!mtaData.config.includeServiceDates}
                                value={getActiveMtaService()?.endDate || ''}
                                onChange={(e) => updateMtaServiceEndDate(activeMtaServiceId, e.target.value)}
                                className="focus:ring-green-500 focus:border-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
                              />
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="mta-observations">Observações do Serviço</Label>
                            <Textarea
                              id="mta-observations"
                              placeholder="Detalhes sobre o andamento, problemas encontrados, etc."
                              value={getActiveMtaService()?.observations || ''}
                              onChange={(e) => updateMtaServiceObservations(activeMtaServiceId, e.target.value)}
                              rows={3}
                              className="focus:ring-green-500 focus:border-green-500"
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex gap-2">
                                <Label htmlFor="mta-image-upload" className="cursor-pointer bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-md text-sm font-medium flex items-center transition-colors">
                                ...
                                <Input
                                  ...
                                  multiple
                                  className="hidden"
                                  onChange={handleMtaImageUpload}
                                />
                                {getActiveMtaService()?.images && getActiveMtaService()!.images.length > 0 && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => removeAllMtaImagesFromService(activeMtaServiceId)}
                            ...
                            {getActiveMtaService()?.images && getActiveMtaService()!.images.length > 0 ? (
                              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
                                {getActiveMtaService()!.images.map((image, index) => (
                                  <div key={index} className="relative group border rounded-lg p-2 bg-gray-50">
                                    <div className="aspect-video relative mb-2 bg-gray-200 rounded overflow-hidden">
                                      ...
                                      />
                                      <button
                                        onClick={() => removeMtaImage(activeMtaServiceId, index)}
                                    ...
                                    <Input
                                      ...
                                      value={image.comment}
                                      onChange={(e) => updateMtaImageComment(activeMtaServiceId, index, e.target.value)}
                                      className="text-xs h-8 focus:ring-green-500 focus:border-green-500"
                                    />
                                  </div>
                                ))}
                              </div>
                              ...
                              <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
                                <FileText className="mx-auto h-12 w-12 text-gray-400" />
                                <p className="mt-2 text-sm text-gray-500">Nenhuma imagem adicionada a este serviço</p>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    {/* Considerações Finais MTA */}
                    <div className="space-y-2">
                      <Label htmlFor="mta-final-considerations" className="text-lg font-semibold">Considerações Finais</Label>
                      <Textarea
                        id="mta-final-considerations"
                        placeholder="Resumo geral da obra, próximos passos, etc."
                        value={mtaData.finalConsiderations}
                        onChange={(e) => updateMtaFinalConsiderations(e.target.value)}
                        rows={4}
                        className="focus:ring-green-500 focus:border-green-500"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4 border p-4 rounded-lg bg-gray-50">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="mta-location">Local/Obra</Label>
                          <Input
                            id="mta-location"
                            value={mtaLocation}
                            onChange={(e) => setMtaLocation(e.target.value)}
                            placeholder="Ex: Edifício Central"
                            className="focus:ring-green-500 focus:border-green-500"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mta-report-name">Título do Relatório</Label>
                          <Input
                            id="mta-report-name"
                            value={mtaReportName}
                            onChange={(e) => setMtaReportName(e.target.value)}
                            placeholder="Ex: Relatório Quinzenal, Vistoria Técnica"
                            className="focus:ring-green-500 focus:border-green-500"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mta-report-desc">Descrição / Subtítulo</Label>
                          <Input
                            id="mta-report-desc"
                            value={mtaReportDescription}
                            onChange={(e) => setMtaReportDescription(e.target.value)}
                      ...
                      {/* Imagens Adicionais MTA */}
                      <div className="space-y-4 border p-4 rounded-lg bg-gray-50">
                        <h4 className="font-medium text-gray-900">Imagens Complementares</h4>

                        <div className="space-y-2">
                          <Label>Imagem de Resultado Final</Label>
                          <Input
                            type="file"
                            accept="image/*"
                            onChange={handleMtaResultImageUpload}
                            className="text-sm"
                          />
                          {mtaData.resultImage && (
                            <div className="flex items-center text-sm text-green-600 mt-1">
                              <CheckCircle className="w-4 h-4 mr-1" />
                              Imagem selecionada: {mtaData.resultImage.name}
                              <button onClick={removeMtaResultImage} className="ml-2 text-red-500 hover:text-red-700">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          )}
                        ...
                        <div className="space-y-2 mt-4">
                          <Label>Termo de Aceite / Assinatura</Label>
                          <Input
                            type="file"
                            accept="image/*"
                            onChange={handleMtaTermsOfServiceImageUpload}
                            className="text-sm"
                          />
                          {mtaData.termsOfServiceImage && (
                            <div className="flex items-center text-sm text-green-600 mt-1">
                              <CheckCircle className="w-4 h-4 mr-1" />
                              Imagem selecionada: {mtaData.termsOfServiceImage.name}
                              <button onClick={removeMtaTermsOfServiceImage} className="ml-2 text-red-500 hover:text-red-700">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          )}
                    ...
                    {/* Resumo e Geração do Relatório MTA */}
                    <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                      <h3 className="text-lg font-semibold mb-4 text-green-900">Resumo do Diário de Obra MTA</h3>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <div className="text-center">
                          <div className="text-lg font-bold text-green-700">{mtaData.services.length}</div>
                          <div className="text-sm text-green-600">Serviços</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-green-700">{getTotalMtaImages()}</div>
                          <div className="text-sm text-green-600">Fotos</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-green-700">{Math.ceil(getTotalMtaImages() / 4)}</div>
                          <div className="text-sm text-green-600">Páginas de Fotos</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-green-700">
                            {mtaLocation ? '✓' : '✗'}
                          </div>
                          <div className="text-sm text-green-600">Local Definido</div>
                        </div>
                      </div>

                      <div className="flex gap-2">
                        <Button
                          onClick={() => generateMtaPDF(true)}
                          disabled={loading || mtaData.services.length === 0}
                  ...
                  <TabsContent value="mark1" className="space-y-6 mt-6">
                          ...
                          {mark1Data.services.map((service, index) => (
                            <div key={service.id} className="flex items-center">
                              <button
                                ...
                                className={`px-4 py-2 text-sm font-medium whitespace-nowrap max-w-xs truncate ${activeMark1ServiceId === service.id
                                  ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700'
                                  : 'text-gray-500 hover:text-gray-700'
                                  }`}
                                title={service.name || `Serviço ${index + 1}`}
                    ...
                    {/* Configuração de Visibilidade das Informações do Relatório Mark1 */}
                    <div className="space-y-4 border-t-2 border-blue-200 pt-6 mt-6">
                      <Label className="text-lg font-semibold">Configurar Informações do Relatório</Label>
                      <p className="text-sm text-gray-600">
                        Selecione quais informações devem aparecer na página de título do Diário de Obra
                      </p>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1Company"
                            checked={mark1Data.config.includeMark1Company ?? true}
                            onChange={(e) => updateMark1Config('includeMark1Company', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1Company" className="text-sm cursor-pointer">Empresa</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1Address"
                            checked={mark1Data.config.includeMark1Address ?? true}
                            onChange={(e) => updateMark1Config('includeMark1Address', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1Address" className="text-sm cursor-pointer">Endereço</Label>
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1Date"
                            checked={mark1Data.config.includeMark1Date ?? true}
                            onChange={(e) => updateMark1Config('includeMark1Date', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1Date" className="text-sm cursor-pointer">Data</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1Location"
                            checked={mark1Data.config.includeMark1Location ?? true}
                            onChange={(e) => updateMark1Config('includeMark1Location', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1Location" className="text-sm cursor-pointer">Local da Obra</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1StartTime"
                            checked={mark1Data.config.includeMark1StartTime ?? true}
                            onChange={(e) => updateMark1Config('includeMark1StartTime', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1StartTime" className="text-sm cursor-pointer">Horário de Início</Label>
                        </div>

                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id="includeMark1EndTime"
                            checked={mark1Data.config.includeMark1EndTime ?? true}
                            onChange={(e) => updateMark1Config('includeMark1EndTime', e.target.checked)}
                            className="rounded"
                          />
                          <Label htmlFor="includeMark1EndTime" className="text-sm cursor-pointer">Horário de Término</Label>
                        </div>
                      </div>
                    </div>

                    {/* Título do Relatório Mark1 */}
                    <div className="space-y-4 border-t-2 border-blue-200 pt-6 mt-6">
                      <Label className="text-lg font-semibold">Título do Relatório</Label>
                      <div className="space-y-2">
                        <Label htmlFor="mark1-report-title">Título Principal</Label>
                        <Input
                          id="mark1-report-title"
                          placeholder="Ex: Relatório Diário de Obras"
                          value={mark1ReportTitle}
                          onChange={(e) => setMark1ReportTitle(e.target.value)}
                          className="text-lg font-semibold"
                        />
                        <p className="text-sm text-gray-600">
                          Este título aparecerá na página de título do relatório
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label htmlFor="mark1-company" className={!mark1Data.config.includeMark1Company ? 'text-gray-400' : ''}>
                            Empresa
                            {!mark1Data.config.includeMark1Company && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-company"
                            ...
                            disabled={!mark1Data.config.includeMark1Company}
                            className={!mark1Data.config.includeMark1Company ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-address" className={!mark1Data.config.includeMark1Address ? 'text-gray-400' : ''}>
                            Endereço
                            {!mark1Data.config.includeMark1Address && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-address"
                            ...
                            disabled={!mark1Data.config.includeMark1Address}
                            className={!mark1Data.config.includeMark1Address ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-date" className={!mark1Data.config.includeMark1Date ? 'text-gray-400' : ''}>
                            Data
                            {!mark1Data.config.includeMark1Date && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-date"
                            type="date"
                            ...
                            disabled={!mark1Data.config.includeMark1Date}
                            className={!mark1Data.config.includeMark1Date ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-location" className={!mark1Data.config.includeMark1Location ? 'text-gray-400' : ''}>
                            Local da Obra
                            {!mark1Data.config.includeMark1Location && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-location"
                            ...
                            disabled={!mark1Data.config.includeMark1Location}
                            className={!mark1Data.config.includeMark1Location ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-start-time" className={!mark1Data.config.includeMark1StartTime ? 'text-gray-400' : ''}>
                            Horário de Início
                            {!mark1Data.config.includeMark1StartTime && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-start-time"
                            type="time"
                            ...
                            disabled={!mark1Data.config.includeMark1StartTime}
                            className={!mark1Data.config.includeMark1StartTime ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-end-time" className={!mark1Data.config.includeMark1EndTime ? 'text-gray-400' : ''}>
                            Horário de Término
                            {!mark1Data.config.includeMark1EndTime && <span className="text-xs text-red-500 ml-2">(Desabilitado na configuração)</span>}
                          </Label>
                          <Input
                            id="mark1-end-time"
                            type="time"
                            ...
                            disabled={!mark1Data.config.includeMark1EndTime}
                            className={!mark1Data.config.includeMark1EndTime ? 'bg-gray-100 text-gray-400' : ''}
                          />
                        </div>
                      </div>
                      <p className="text-sm text-gray-600">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                      ...
                      <div className="flex gap-2">
                        <Button
                          onClick={() => generateMark1PDF(true)}
                          disabled={loading || mark1Data.services.length === 0}
                          className="flex-1 bg-blue-100 text-blue-700 hover:bg-blue-200 border border-blue-200"
                          size="lg"
                          variant="outline"
                        >
                          <Eye className="w-5 h-5 mr-2" />
                        ...
                        <Button
                          onClick={() => generateMark1PDF(false)}
                          disabled={loading || mark1Data.services.length === 0}
                  ...
                  <TabsContent value="processo-mark1" className="space-y-6 mt-6">
                    {/* Cabeçalho do Relatório de Processo Mark1 */}
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
                      <h3 className="text-xl font-bold text-blue-900 mb-2 flex items-center">
                        <Camera className="w-6 h-6 mr-2" />
                        Relatório de Processo da Obra - Mark1
                      </h3>
                      <p className="text-blue-700">
                        Crie um relatório de processo com o layout premium da Mark1, organizando fotos por fases (Antes/Durante/Depois)
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label htmlFor="mark1-process-work-name">Nome da Obra *</Label>
                          <Input
                            id="mark1-process-work-name"
                            placeholder="Ex: Reforma do Edifício Central"
                            value={mark1ProcessReportData.workName}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, workName: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-process-work-date">Data da Obra</Label>
                          <Input
                            id="mark1-process-work-date"
                            type="date"
                            value={mark1ProcessReportData.workDate}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, workDate: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-process-company">Cliente / Empresa</Label>
                          <Input
                            id="mark1-process-company"
                            placeholder="Ex: Cliente S/A"
                            value={mark1ProcessReportData.company}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, company: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="mark1-process-location">Local</Label>
                          <Input
                            id="mark1-process-location"
                            placeholder="Ex: Edifício Central"
                            value={mark1ProcessReportData.location}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, location: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          <Label htmlFor="mark1-process-address">Endereço</Label>
                          <Input
                            id="mark1-process-address"
                            placeholder="Ex: Rua Principal, 123"
                            value={mark1ProcessReportData.address}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, address: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          <Label htmlFor="mark1-process-description">Descrição do Relatório</Label>
                          <Textarea
                            id="mark1-process-description"
                            placeholder="Descrição ou informações adicionais que aparecerão na primeira página..."
                            value={mark1ProcessReportData.description}
                            onChange={(e) => setMark1ProcessReportData(prev => ({ ...prev, description: e.target.value }))}
                            rows={3}
                          />
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <Label className="text-lg font-semibold">
                        Fotos do Processo ({mark1ProcessReportData.processImages.length} total)
                      </Label>
                      <p className="text-sm text-gray-600">
                        Organize as fotos por fase da obra para mostrar a evolução do trabalho
                      </p>
                      ...
                      <Tabs defaultValue="antes" className="w-full">
                        <TabsList className="grid w-full grid-cols-3">
                          <TabsTrigger value="antes" className="flex items-center gap-2">
                            <span className="w-3 h-3 bg-blue-500 rounded-full"></span>
                            Antes ({getMark1ProcessReportImagesByPhase('antes').length})
                          </TabsTrigger>
                          <TabsTrigger value="durante" className="flex items-center gap-2">
                            <span className="w-3 h-3 bg-orange-500 rounded-full"></span>
                            Durante ({getMark1ProcessReportImagesByPhase('durante').length})
                          </TabsTrigger>
                          <TabsTrigger value="depois" className="flex items-center gap-2">
                            <span className="w-3 h-3 bg-green-500 rounded-full"></span>
                            Depois ({getMark1ProcessReportImagesByPhase('depois').length})
                          </TabsTrigger>
                        </TabsList>
                          ...
                          <TabsContent key={phase} value={phase} className="space-y-4">
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                              <div className="text-center">
                                <Camera className="mx-auto h-8 w-8 text-gray-400" />
                                <div className="mt-2">
                                  <Label htmlFor={`mark1-process-report-${phase}`} className="cursor-pointer">
                                    <span className="text-sm font-medium text-gray-900">
                                      Adicionar fotos do {phase.toUpperCase()}
                                    </span>
                                    <Input
                                      id={`mark1-process-report-${phase}`}
                                      type="file"
                                      multiple
                                      accept="image/*"
                                      onChange={(e) => handleMark1ProcessReportImageUpload(e, phase as any)}
                                      className="hidden"
                                    />
                                  </Label>
                                </div>
                            ...
                            {getMark1ProcessReportImagesByPhase(phase as any).length > 0 && (
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {getMark1ProcessReportImagesByPhase(phase as any).map((imageData, index) => {
                                  const globalIndex = mark1ProcessReportData.processImages.findIndex(img => img === imageData);
                                  return (
                                    <div key={index} className="space-y-2">
                                      <div className="relative">
                                        <img
                                        ...
                                        <Button
                                          ...
                                          size="sm"
                                          className="absolute top-1 right-1 h-5 w-5 p-0"
                                          onClick={() => removeMark1ProcessReportImage(globalIndex)}
                                        >
                                          <Trash2 className="h-2 w-2" />
                                        </Button>
                                        <span className={`absolute bottom-1 left-1 text-white text-xs px-1 rounded ${phase === 'antes' ? 'bg-blue-600' : phase === 'durante' ? 'bg-orange-600' : 'bg-green-600'
                                          }`}>
                                          {phase.toUpperCase()} {index + 1}
                                        </span>
                                      </div>
                                      <Input
                                        placeholder="Comentário da foto (opcional)"
                                        value={imageData.comment}
                                        onChange={(e) => updateMark1ProcessReportImageComment(globalIndex, e.target.value)}
                                        className="text-xs"
                                      />
                                    </div>
                                  );
                    ...
                    {/* Resumo e Geração do Relatório */}
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                      <h3 className="text-lg font-semibold mb-4 text-blue-900">Resumo do Relatório de Processo Mark1</h3>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <div className="text-center">
                          <div className="text-lg font-bold text-blue-700">{getMark1ProcessReportImagesByPhase('antes').length}</div>
                          <div className="text-sm text-blue-600">Fotos Antes</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-blue-700">{getMark1ProcessReportImagesByPhase('durante').length}</div>
                          <div className="text-sm text-blue-600">Fotos Durante</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-blue-700">{getMark1ProcessReportImagesByPhase('depois').length}</div>
                          <div className="text-sm text-blue-600">Fotos Depois</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-blue-700">{mark1ProcessReportData.processImages.length}</div>
                          <div className="text-sm text-blue-600">Total de Fotos</div>
                        </div>
                      </div>

                      <Button
                        onClick={generateMark1ProcessReportPDF}
                        disabled={loading || mark1ProcessReportData.processImages.length === 0 || !mark1ProcessReportData.workName.trim()}
                        className="w-full bg-blue-600 hover:bg-blue-700"
                        size="lg"
                      >
                        <Download className="w-5 h-5 mr-2" />
                        {loading ? 'Gerando Relatório...' : 'Gerar Relatório de Processo Mark1 PDF'}
                      </Button>
      ...
      {isPreviewOpen && previewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200">
